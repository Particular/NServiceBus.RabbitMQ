...
:: restore packages
%NUGET_LOCAL_DIR%\NuGet.exe restore .\packages.config -PackagesDirectory ./script-packages -MSBuildVersion 15 -Verbosity quiet

:: find VS install path
for /f "usebackq tokens=*" %%i in (`script-packages\vswhere.1.0.62\tools\vswhere -latest -products * -requires Microsoft.Component.MSBuild -property installationPath`) do (
  set VS_INSTALL_PATH=%%i
)

:: run script
"%VS_INSTALL_PATH%\MSBuild\15.0\Bin\Roslyn\csi.exe" reset-virtual-host.csx %*