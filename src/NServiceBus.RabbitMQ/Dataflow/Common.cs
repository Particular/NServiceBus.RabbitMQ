...
namespace System.Threading.Tasks.Dataflow.Internal
{
    ...
    static class Common
    {
            ...
            TStateIn stateIn, out TStateOut stateOut)
        {
            ...
            for (var c = ITERATION_LIMIT; c > 0; c--)
            {
                if (!Thread.Yield())
                {
        ...
        internal static int GetBlockId(IDataflowBlock block)
        {
            ...
            var t = GetPotentiallyNotSupportedCompletionTask(block);
            ...
            CancellationToken cancellationToken, Task completionTask, Action<object> completeAction, object completeState)
        {
            ...
            else if (cancellationToken.CanBeCanceled)
            {
                ...
                completionTask.ContinueWith((completed, state) => ((CancellationTokenRegistration) state).Dispose(),
                    reg, cancellationToken, GetContinuationOptions(), TaskScheduler.Default);
            }
        }
        ...
        /// <param name="targetInnerExceptions">
        /// Whether to store the data into the exception's inner exception(s) in addition to
        ...
        internal static void StoreDataflowMessageValueIntoExceptionData<T>(Exception exc, T messageValue, bool targetInnerExceptions = false)
        {
            ...
            StoreStringIntoExceptionData(exc, EXCEPTIONDATAKEY_DATAFLOWMESSAGEVALUE, strValue);
            ...
            if (targetInnerExceptions)
            {
                ...
                if (aggregate != null)
                {
                    ...
                    {
                        StoreStringIntoExceptionData(innerException, EXCEPTIONDATAKEY_DATAFLOWMESSAGEVALUE, strValue);
                    }
                }
                // Otherwise, if there's an Exception.InnerException, store into that.
                else if (exc.InnerException != null)
                {
                    StoreStringIntoExceptionData(exc.InnerException, EXCEPTIONDATAKEY_DATAFLOWMESSAGEVALUE, strValue);
                }
            }
        }
        ...
        static void StoreStringIntoExceptionData(Exception exception, string key, string value)
        {
            ...
            try
            {
                ...
                if (!data.IsFixedSize && !data.IsReadOnly && data[key] == null)
                {
                    data[key] = value;
                }
            }
        ...
        static Task<bool> CreateCachedBooleanTask(bool value)
        {
            ...
            // is still valid.
            var atmb = AsyncTaskMethodBuilder<bool>.Create();
        ...
        {
            var atmb = AsyncTaskMethodBuilder<TResult>.Create();
        ...
        /// <summary>
        /// Gets the completion task of a block, and protects against common cases of the completion task not being
        ...
        /// <summary>
        /// Starts an already constructed task with handling and observing exceptions that may come from the scheduling
        /// process.
        /// </summary>
        /// <param name="task">Task to be started.</param>
        /// <param name="scheduler">TaskScheduler to schedule the task on.</param>
        /// <returns>
        /// null on success, an exception reference on scheduling error. In the latter case, the task reference is nulled
        /// out.
        /// </returns>
            ...
            ref List<Exception> exceptions)
        {
            ...
            var initialCount = postponedMessages.Count;
        ...
        /// Handler for exceptions from the target. May be null which would propagate the exception
        /// to the caller.
        /// </param>
        ...
        static void PropagateCompletionAsContinuation(Task sourceCompletionTask, IDataflowBlock target)
        {
            Contract.Requires(sourceCompletionTask != null, "sourceCompletionTask may not be null.");
            Contract.Requires(target != null, "The target where completion is to be propagated may not be null.");
            sourceCompletionTask.ContinueWith((task, state) => PropagateCompletion(task, (IDataflowBlock) state, AsyncExceptionHandler),
                target, CancellationToken.None, GetContinuationOptions(), TaskScheduler.Default);
        }

        /// <summary>
        /// Propagates completion of sourceCompletionTask to target based on sourceCompletionTask's current state. See
        /// PropagateCompletion.
        /// </summary>
        ...
        static class CachedGenericDelegates<T>
        {
            ...
            internal static readonly Action<object, TargetRegistry<T>, ITargetBlock<T>> CreateUnlinkerShimAction =
                (syncObj, registry, target) => { lock (syncObj) registry.Remove(target); };
        }
    }
    ...
    class BoundingStateWithPostponed<TInput> : BoundingState
    {
        /// <summary>Initializes the BoundingState.</summary>
        /// <param name="boundedCapacity">The positive bounded capacity.</param>
    ...
    /// <typeparam name="TInput">Specifies the type of input messages.</typeparam>
    class BoundingStateWithPostponedAndTask<TInput> : BoundingStateWithPostponed<TInput>
    {
        /// <summary>Initializes the BoundingState.</summary>
        /// <param name="boundedCapacity">The positive bounded capacity.</param>
