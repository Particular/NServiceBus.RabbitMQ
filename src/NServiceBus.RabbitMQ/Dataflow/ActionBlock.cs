...
namespace System.Threading.Tasks.Dataflow
{
    ...
    /// <summary>
    /// Provides a dataflow block that invokes a provided <see cref="System.Action{T}" /> delegate for every data
    /// element received.
    /// </summary>
    ...
    sealed class ActionBlock<TInput> : ITargetBlock<TInput>, IDebuggerDisplay
    {
        ...
        /// <summary>
        /// Initializes the <see cref="ActionBlock{T}" /> with the specified <see cref="System.Action{T}" /> and
        ...
        /// <summary>
        /// Initializes the <see cref="ActionBlock{T}" /> with the specified <see cref="System.Func{T,Task}" /> and
        /// <see cref="ExecutionDataflowBlockOptions" />.
        /// </summary>
        /// <param name="action">The action to invoke with each data element received.</param>
        /// <param name="dataflowBlockOptions">The options with which to configure this <see cref="ActionBlock{T}" />.</param>
        ...
        ActionBlock(Delegate action, ExecutionDataflowBlockOptions dataflowBlockOptions)
        {
            ...
            else
            {
                ...
                Common.WireCancellationToComplete(
                    dataflowBlockOptions.CancellationToken, Completion, state => ((TargetCore<TInput>) state).Complete(null, true), _defaultTarget);
            }
#if FEATURE_TRACING
        ...
        {
            get { return $"{Common.GetNameForDebugger(this, _defaultTarget != null ? _defaultTarget.DataflowBlockOptions : _spscTarget.DataflowBlockOptions)}, InputCount={InputCountForDebugger}"; }
        }
        ...
        public void Complete()
        {
            ...
            {
                _defaultTarget.Complete(null, false);
            }
            else
            {
        ...
        void IDataflowBlock.Fault(Exception exception)
        {
            ...
            if (_defaultTarget != null)
            {
                _defaultTarget.Complete(exception, true);
            }
            else
            {
        ...
        void ProcessMessageWithTask(Func<TInput, Task> action, KeyValuePair<TInput, long> messageWithId)
        {
            ...
            if (task == null)
            {
                ...
                if (caughtException != null && !Common.IsCooperativeCancellation(caughtException))
                {
                    ...
                    _defaultTarget.Complete(caughtException, true, true, false);
                }
            ...
            else
            {
                ...
                task.ContinueWith((completed, state) => { ((ActionBlock<TInput>) state).AsyncCompleteProcessMessageWithTask(completed); }, this, CancellationToken.None, Common.GetContinuationOptions(TaskContinuationOptions.ExecuteSynchronously), TaskScheduler.Default);
            }
        }
        ...
        void AsyncCompleteProcessMessageWithTask(Task completed)
        {
            ...
            {
                _defaultTarget.Complete(completed.Exception, true, true, true);
            }
