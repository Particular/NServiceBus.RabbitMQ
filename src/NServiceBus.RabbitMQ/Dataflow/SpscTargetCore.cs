...
namespace System.Threading.Tasks.Dataflow.Internal
{
    ...
    /// <summary>
    /// Provides a core implementation of <see cref="ITargetBlock{TInput}" /> for use when there's only a single producer
    /// posting data.
    /// </summary>
    /// <typeparam name="TInput">Specifies the type of data accepted by the <see cref="TargetCore{TInput}" />.</typeparam>
    [DebuggerDisplay("{DebuggerDisplayContent,nq}")]
    sealed class SpscTargetCore<TInput>
    {
        ...
        /// true if we need to call back to the source to consume the message; otherwise, false if we
        /// can simply accept it directly.
        /// </param>
        ...
        void ProcessMessagesLoopCore()
        {
            ...
            var maxMessagesToProcess = _dataflowBlockOptions.ActualMaxMessagesPerTask;
            ...
            while (continueProcessing)
            {
                ...
                var nextMessage = default(TInput);
                try
                {
                ...
                catch (Exception exc)
                {
                    ...
                    if (!Common.IsCooperativeCancellation(exc))
                    {
                        ...
                        Common.StoreDataflowMessageValueIntoExceptionData(exc, nextMessage, false);
                        StoreException(exc);
                    }
                }
                finally
                {
                    ...
                    else
                    {
                        ...
                        var wasDecliningPermanently = _decliningPermanently;
        ...
        readonly SingleProducerSingleConsumerQueue<TInput> _messages = new SingleProducerSingleConsumerQueue<TInput>();
        ...
        internal sealed class DebuggingInformation
        {
            ...
            {
                get { return _target._activeConsumer != null && !_target.Completion.IsCompleted ? 1 : 0; }
            }
            ...
            readonly SpscTargetCore<TInput> _target;
        }
    }
}