...
namespace System.Threading.Tasks.Dataflow.Internal
{
    ...
    /// <summary>
    /// Provides a core implementation of <see cref="ITargetBlock{TInput}" />.
    /// </summary>
    ...
    sealed class TargetCore<TInput>
    {
        ...
        /// The options to use to configure this block. The target core assumes these options
        /// are immutable.
        /// </param>
            ...
            TargetCoreOptions targetCoreOptions)
        {
            ...
            if (_dataflowBlockOptions.BoundedCapacity != Dataflow.DataflowBlockOptions.Unbounded)
            {
        ...
        readonly TaskCompletionSource<VoidResult> _completionSource = new TaskCompletionSource<VoidResult>();
        ...
        readonly IProducerConsumerQueue<KeyValuePair<TInput, long>> _messages;
        ...
        int _keepAliveBanCounter;

        /// <summary>The next available ID we can assign to a message about to be processed.</summary>
        PaddedInt64 _nextAvailableInputMessageId; // initialized to 0... very important for a reordering buffer
        ...
        /// <param name="storeExceptionEvenIfAlreadyCompleting">
        /// If true, an exception will be stored after _decliningPermanently
...
#pragma warning disable 0420
        ...
        internal DataflowMessageStatus OfferMessage(DataflowMessageHeader messageHeader, TInput messageValue, ISourceBlock<TInput> source, bool consumeToAccept)
        {
            ...
            lock (IncomingLock)
            {
                    ...
                    (_boundingState.OutstandingTransfers == 0 && _boundingState.CountIsLessThanBound && _boundingState.PostponedMessages.Count == 0))
                {
                    ...
                    var messageId = _nextAvailableInputMessageId.Value++;
        ...
        {
            get { return _messages.GetCountSafe(IncomingLock); }
        }
        ...
        /// <summary>Signals to the target core that a previously launched asynchronous operation has now completed.</summary>
        /// <param name="boundingCountChange">
        /// The number of elements by which to change the bounding count, if bounding is
        /// occurring.
        /// </param>
        ...
        bool UsesAsyncCompletion
        {
            get { return (_targetCoreOptions & TargetCoreOptions.UsesAsyncCompletion) != 0; }
        }
        ...
        bool HasRoomForMoreOperations
        {
            get
            {
                ...
                Common.ContractAssertMonitorStatus(IncomingLock, true);
        ...
        void ProcessAsyncIfNecessary_Slow(bool repeat)
        {
            ...
            var messagesAvailableOrPostponed =
                !_messages.IsEmpty ||
            ...
            if (messagesAvailableOrPostponed && !CanceledOrFaulted)
            {
                ...
                if (exception != null)
                {
                    ...
                    Task.Factory.StartNew(exc => Complete((Exception) exc, true, true, false, true),
        ...
        void ProcessMessagesLoopCore()
        {
            ...
            var messageWithId = default(KeyValuePair<TInput, long>);
            try
            {
                var useAsyncCompletion = UsesAsyncCompletion;
                var shouldAttemptPostponedTransfer = _boundingState != null && _boundingState.BoundedCapacity > 1;
                var numberOfMessagesProcessedByThisTask = 0;
                var numberOfMessagesProcessedSinceTheLastKeepAlive = 0;
                ...
                while (numberOfMessagesProcessedByThisTask < maxMessagesPerTask && !CanceledOrFaulted)
                {
                    ...
                    if (shouldAttemptPostponedTransfer &&
                        TryConsumePostponedMessage(true, out transferMessageWithId))
                    {
                        lock (IncomingLock)
                        {
                            Debug.Assert(
                    ...
                    else
                    {
                        ...
                        if (!TryGetNextAvailableOrPostponedMessage(out messageWithId))
                        {
                            ...
                            if (_dataflowBlockOptions.MaxDegreeOfParallelism != 1)
                            {
                                break;
                            }
        ...
        bool TryGetNextAvailableOrPostponedMessage(out KeyValuePair<TInput, long> messageWithId)
        {
            ...
            if (_boundingState != null && TryConsumePostponedMessage(false, out messageWithId))
            {
                return true;
            }
        ...
        bool TryConsumePostponedMessage(
            bool forPostponementTransfer,
            out KeyValuePair<TInput, long> result)
        {
            Contract.Requires(
                _dataflowBlockOptions.BoundedCapacity !=
                Dataflow.DataflowBlockOptions.Unbounded, "Only valid to use when in bounded mode.");
            Common.ContractAssertMonitorStatus(IncomingLock, false);

            // Iterate until we either consume a message successfully or there are no more postponed messages.
            var countIncrementedExpectingToGetItem = false;
            var messageId = Common.INVALID_REORDERING_ID;
            while (true)
            {
            ...
            if (_reorderingBuffer != null && messageId != Common.INVALID_REORDERING_ID)
            {
        ...
        bool CanceledOrFaulted
        {
            get { return _dataflowBlockOptions.CancellationToken.IsCancellationRequested || Volatile.Read(ref _exceptions) != null; }
        }
        ...
        void CompleteBlockIfPossible()
        {
            ...
            var noMoreMessages = _decliningPermanently && _messages.IsEmpty;
    ...
    // then we shouldn't log just for the first half of the block; the source half will handle logging.
        ...
        {
            get { return _boundingState != null; }
        }
        ...
        internal sealed class DebuggingInformation
        {
            ...
            {
                get { return _target._messages.Select(kvp => kvp.Key).ToList(); }
            }
            ...
            {
                get { return _target._numberOfOutstandingOperations - _target._numberOfOutstandingServiceTasks; }
            }
