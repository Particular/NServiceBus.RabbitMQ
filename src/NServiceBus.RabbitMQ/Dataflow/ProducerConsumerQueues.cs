...
namespace System.Threading.Tasks
{
    ...
    interface IProducerConsumerQueue<T> : IEnumerable<T>
    {
        ...
        /// <summary>
        /// A thread-safe way to get the number of items in the collection. May synchronize access by locking the provided
    ...
    sealed class MultiProducerMultiConsumerQueue<T> : ConcurrentQueue<T>, IProducerConsumerQueue<T>
    {
        /// <summary>Enqueues an item into the queue.</summary>
        /// <param name="item">The item to enqueue.</param>
    ...
    sealed class SingleProducerSingleConsumerQueue<T> : IProducerConsumerQueue<T>
    {
        ...
        {
            for (var segment = _head; segment != null; segment = segment._next)
            {
                for (var pt = segment._state._first;
        ...
        /// <param name="result">The dequeued item.</param>
        /// <returns>true if an item could be dequeued; otherwise, false.</returns>
        bool TryDequeueSlow(ref Segment segment, ref T[] array, out T result)
        {
            Contract.Requires(segment != null, "Expected a non-null segment.");
        ...
        /// <param name="result">The peeked item.</param>
        /// <returns>true if an item could be peeked; otherwise, false.</returns>
        bool TryPeekSlow(ref Segment segment, ref T[] array, out T result)
        {
            Contract.Requires(segment != null, "Expected a non-null segment.");
            Contract.Requires(array != null, "Expected a non-null item array.");
        ...
        /// The predicate that must return true for the item to be dequeued.  If null, all items implicitly
        /// return true.
        /// </param>
        /// <param name="result">The dequeued item.</param>
        /// <returns>true if an item could be dequeued; otherwise, false.</returns>
        ...
        /// <param name="result">The dequeued item.</param>
        /// <returns>true if an item could be dequeued; otherwise, false.</returns>
        bool TryDequeueIfSlow(Predicate<T> predicate, ref Segment segment, ref T[] array, out T result)
        {
            Contract.Requires(segment != null, "Expected a non-null segment.");
            Contract.Requires(array != null, "Expected a non-null item array.");
        ...
        const int INIT_SEGMENT_SIZE = 32; // must be a power of 2

        /// <summary>The maximum size to use for segments (in number of elements).</summary>
        const int MAX_SEGMENT_SIZE = 0x1000000; // this could be made as large as Int32.MaxValue / 2
