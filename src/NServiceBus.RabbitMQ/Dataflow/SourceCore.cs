...
namespace System.Threading.Tasks.Dataflow.Internal
{
    ...
    sealed class SourceCore<TOutput>
    {
        ...
        /// Action to invoke in order to decline the associated target half, which will in turn
        /// decline this source core.
        /// </param>
        ...
        {
            get { lock (OutgoingLock) lock (ValueLock) return _messages.Count; }
        }
        ...
        internal TOutput ConsumeMessage(DataflowMessageHeader messageHeader, ITargetBlock<TOutput> target, out bool messageConsumed)
        {
            ...
            lock (OutgoingLock)
            {
                ...
                lock (ValueLock)
                {
                    ...
                    _nextMessageReservedFor = null;
                    _targetRegistry.Remove(target, true);
        ...
        internal void AddMessages(IEnumerable<TOutput> items)
        {
            ...
            if (itemsAsList != null)
            {
                for (var i = 0; i < itemsAsList.Count; i++)
                {
            ...
            {
                var itemsAsArray = items as TOutput[];
        ...
        internal void AddAndUnwrapAggregateException(AggregateException aggregateException)
        {
            ...
            lock (ValueLock)
            {
                Common.AddException(ref _exceptions, aggregateException, true);
            }
        }
        ...
        {
            Common.ContractAssertMonitorStatus(OutgoingLock, true);
            Common.ContractAssertMonitorStatus(ValueLock, false);
            ...
            if (header.IsValid)
            {
                ...
                else
                {
                    ...
                    {
                        var next = cur.Next;
                        if (OfferMessageToTarget(header, message, cur.Target, out messageWasAccepted))
                        {
                            break;
                        }
                        cur = next;
            ...
            if (messageWasAccepted)
            {
                ...
                if (_itemsRemovedAction != null)
                {
                    var count = _itemCountingFunc != null ? _itemCountingFunc(_owningSource, message, null) : 1;
                    _itemsRemovedAction(_owningSource, count);
                }
            }
            ...
            out bool messageWasAccepted)
        {
            ...
            var result = target.OfferMessage(header, message, _owningSource, false);
            ...
            if (result == DataflowMessageStatus.DecliningPermanently)
            {
        ...
        /// <param name="outgoingLockKnownAcquired">
        /// Whether the caller is sure that the outgoing lock is currently held by this
        /// thread.
        /// </param>
        void OfferAsyncIfNecessary(bool isReplacementReplica, bool outgoingLockKnownAcquired)
        {
            Common.ContractAssertMonitorStatus(ValueLock, true);
...
#pragma warning restore 0420
        ...
        readonly TaskCompletionSource<VoidResult> _completionTask = new TaskCompletionSource<VoidResult>();
        ...
        /// </remarks>
        readonly SingleProducerSingleConsumerQueue<TOutput> _messages = new SingleProducerSingleConsumerQueue<TOutput>(); // protected by AddMessage/ValueLock
        ...
        internal sealed class DebuggingInformation
        {
            ...
            {
                get { return _source._decliningPermanently; }
            }
