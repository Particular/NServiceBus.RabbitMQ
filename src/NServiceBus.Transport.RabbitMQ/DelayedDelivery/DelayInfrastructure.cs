    ...
    static class DelayInfrastructure
    {
        ...
        public static async Task Build(IChannel channel, CancellationToken cancellationToken = default)
        {
            ...
            for (var level = MaxLevel; level >= 0; level--)
            {
                ...
                await channel.ExchangeDeclareAsync(currentLevel, ExchangeType.Topic, true).ConfigureAwait(false);
            ...
            await channel.ExchangeDeclareAsync(DeliveryExchange, ExchangeType.Topic, true).ConfigureAwait(false);
        ...
        public static async Task TearDown(IChannel channel, CancellationToken cancellationToken = default)
        {
            ...
            for (var level = MaxLevel; level >= 0; level--)
            {
                ...
                // TODO cancellation token
                await channel.QueueDeleteAsync(name).ConfigureAwait(false);
