namespace NServiceBus.Transport.RabbitMQ
{
    ...
    {
        public MessagePumpConnectionFailedCircuitBreaker(string name, TimeSpan timeToWaitBeforeTriggering, Action<Exception> criticalErrorAction)
        {
            this.name = name;
        ...
        void CircuitBreakerTriggered(object state)
        {
            if (Interlocked.Read(ref failureCount) > 0)
            {
                ...
                criticalErrorAction(lastException);
            }
        }
        ...
        Timer timer;
        Action<Exception> criticalErrorAction;
        long failureCount;
